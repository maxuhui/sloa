$(function () {
    var dg = $("#roleGrid");
    var resourceTree = $("#rolePermissionTree");
    var resourcePanel = $("#rolePermissionPanel");
    var currentRoelId;
    ...
    $("#rolePermissionSave").on("click", function () {
        if (currentRoelId) {
            ...
            var prams = "roleId=" + currentRoelId + "&permissionId=" + resourceId.join("&permissionId=")
            // 发送请求保存关系
            $.post("system/role/permission/save", prams, function (rsp) {
                if (rsp.success) {
                    dg.datagrid("reload");
